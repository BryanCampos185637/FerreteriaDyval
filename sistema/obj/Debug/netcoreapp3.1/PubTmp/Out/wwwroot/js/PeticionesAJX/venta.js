...
function callTable() {
    paintTable('/venta/listDetalleVenta', ['producto', 'cant', 'P.U', 'comi','p.com', 'desc', 'p.des','subt']);
...
*/
    ...
    $.get(link, function (data) {
        ...
        $.each(data, function (obj, item) {
            ...
            html += '<td class="text-center">';
            html += '<a title="Eliminar producto de la lista" class="btn-sm btn-danger" href="#" onclick="deleteProducto(' + item.iidproducto + ')"><i class="fas fa-trash"></i></a> ';
            html += '</td>';
            html += '</tr>';
        ...
        if (result.isConfirmed) {
            $.get('/venta/cancelarVenta', function (e) {
                if (e > 0) {
                    ...
                    document.getElementById('totalVenta').innerHTML = '$0.00';
                    callTable();
                    messeges('success', 'Venta cancelada');
                    limpiar();
                } else {
...
function mostrarPosiblesProductosParDescuentoGeneral(link, headboard) {
    $.get(link, function (data) {
        var html = ""; var totalComision = 0; var totalFactura = 0;
        var totalComisionProductosListados = 0; var totalFacturaProductosListados = 0;
        ...
        $.each(data, function (obj, item) {
            ...
            if (!esSubProducto) {//solo se muestra si es un subproducto
                ...
                html += '<td>' + item.nombreproducto + '</td>';
                html += '<td>' + item.cantidad + '</td>';
                html += '<td>' + item.preciounitario.toFixed(4) + '</td>';
                html += '<td>' + item.comision.toFixed(4) + '</td>';
                html += '<td>' + item.pcomision + '%</td>';
                html += '<td>' + item.descuento.toFixed(4) + '</td>';
                html += '<td>' + item.pdescuento + '%</td>';
        ...
        var inputtotal = document.getElementById('txtActual').value;
        if (inputtotal == '') {
            document.getElementById('txtActual').value = totalFactura.toFixed(4);
            document.getElementById('txtComisionActual').value = totalComision.toFixed(4);
        ...
        //else {
        //    document.getElementById('txtTotalNuevo').value = total;
        //}
...
function calculateDescuentoGeneral() {
    ...
    var totalComisionProductosListados = parseFloat(document.getElementById('totalComisionProductosListados').value);
        ...
        if (!isNaN(porcentajeDescuento)) {
            if (porcentajeDescuento > 0) {//si el porcentaje es mayor a 0 
                if (comisionActual == 0) {//si la comision es igual a 0 entonces se saca descuento al total de la factura
                    ...
                    var precioNuevo = totalActual - descuento;
                    document.getElementById('txtTotalNuevo').value = precioNuevo.toFixed(4);
                } else {//si no se le saca descuento a la comision
                    var descuento = (totalComisionProductosListados / 100) * porcentajeDescuento;
                    document.getElementById('txtDescuentoGeneral').value = descuento.toFixed(4);
                    var comisionNueva = comisionActual - descuento;
                    if (comisionNueva <= 0) {
                        document.getElementById('txtComisionNuevo').value = 0;
                        var precioNuevo = totalActual - comisionActual;
                        document.getElementById('txtTotalNuevo').value = precioNuevo.toFixed(4);
                    }
                    else {
                        var precioNuevo = totalActual - descuento;
